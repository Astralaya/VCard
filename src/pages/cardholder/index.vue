      ...
      <div class="weui-search-bar">
        <div class="weui-search-bar__form">
          <div class="weui-search-bar__box">
            ...
            <input type="text" confirm-type="search" class="weui-search-bar__input" placeholder="搜索" v-model="inputVal" :focus="inputShowed" @input="inputTyping" @confirm="searchData"/>
      ...
      <div class="weui-cells weui-cells_after-title" v-for="(item , index) in list" :key="index">
        <div class=" weui-cell_access" hover-class="weui-cell_active">
          <div class="weui-cell">
            ...
            <div class="weui-cell__ft " @click="call(item)">
...
export default {
  ...
  methods: {
    ...
    async getData() {
      ...
      if(this.inputVal != '') {
        par = Object.assign(par , {'@key' : this.inputVal})
      }
    ...
    async call(item) {
      if (item.strMobile) {
        wx.makePhoneCall({
          phoneNumber: item.strMobile
        });
        var par = {
          strOpenId_b: item.strOpenId,
          strOpenId_c: this.userInfo.strOpenId,
          strMobile: item.strMobile
        };
        const res = await api.post_Call(par);
      } else {
        api.wxToast({
          title: "该用户没有设置电话号码"
        });
